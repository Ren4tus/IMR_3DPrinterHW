...
public class ViewStructure_new : MonoBehaviour
{
    ...
    void OnEnable()
    {
        InitializeTransform(initialTranslations.Length - 1);
        currentObjIdx = initialTranslations.Length - 1;
    }
    ...
    public void InitializeTransformByScriptButton()
    {
        if (MainController.GetCurrentSeq() + 1 == MainController.NumberOfSeq)
            InitializeTransform(currentObjIdx);            
     }
    ...
    public void ChangeStructure(int a)
    {
        if (MainController.GetCurrentSeq() + 1 == MainController.NumberOfSeq)
        {
            ...
            m_ShootLocation = gameObjects[a].transform;
            InitializeTransform(a);

            currentObjIdx = a;
        }
        else {
            MainController.LoadSeq(seq[a], true);
        }
    }
